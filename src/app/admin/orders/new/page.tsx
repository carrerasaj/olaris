...
export default async function NewOrderPage({
  ...
  if (!customerId) {
    // No customer selected — send them to pick one.
  ...
  async function handleSubmit(
    ...
    const result = await createOrderAction(
      uiOrderToInput(ui, custId, companyId),
    )
    return result
  }
    ...
    <div className="adm-page">
      <div className="adm-pageheader">
        <div>
          ...
          <h1 style={{ marginTop: 4 }}>New order — pick a customer</h1>
          <div className="sub">Orders are always attached to a customer</div>
        </div>
      ...
      <div className="adm-card">
        {list.length === 0 ? (
          <div className="adm-empty">
            ...
            <p>Add a customer before creating an order.</p>
            <Link
              ...
              {list.map((c) => (
                <tr key={c.id}>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <Link
                      href={`/admin/orders/new?customerId=${c.id}`}
