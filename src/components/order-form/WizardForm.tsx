...
/**
 * Multi-step wizard order variant (customer-facing).
 * Ported from src/components/mockups/form-variant-b.jsx.
 */
...
interface WizardStep {
  id: 'customer' | 'vehicle' | 'options' | 'pricing' | 'finance' | 'delivery' | 'review'
  label: string
  num: string
}
...
export function WizardForm({
  ...
  const go = (d: number) => setStep((s) => Math.max(0, Math.min(totalSteps - 1, s + d)))
          ...
          <div className="ol-step-fade" key={step}>
            {currentStep.id === 'customer' && (
              <SectionCard
                ...
                sub="Individual or business — the VAT position depends on this."
              >
            ...
            {currentStep.id === 'options' && (
              <SectionCard
                ...
                sub="Each line is VAT-rated individually"
              >
            ...
            {currentStep.id === 'pricing' && (
              <SectionCard
                ...
                sub="Auto-calculates VAT and on-the-road costs"
              >
            ...
            {currentStep.id === 'review' && (
              <>
                <SectionCard
                  title="Review order"
                  sub="Confirm everything before e-signature is requested"
                  right={<Pill tone="amber">Final step</Pill>}
                >
                ...
                <SectionCard title="E-signature" sub="Olaris signing envelope · 2 recipients">
                  <div className="ol-stack" style={{ gap: 14 }}>
                    <OlarisEnvelopeStrip
                      envelopeRef={`ENV-${orderRef.slice(-8)}`}
                      recipients={2}
                    />
                    <SignatureBlock
            ...
            <div className="ol-formfooter" style={{ marginTop: 10 }}>
              ...
              {step < totalSteps - 1 ? (
                <button type="button" className="ol-btn ol-btn-primary" onClick={() => go(1)}>
                  Next step
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round">
                ...
                <button type="button" className="ol-btn ol-btn-primary" onClick={onSubmit}>
          ...
          <aside>
            <div className="ol-summary" style={{ top: 24 }}>
              ...
              <div className="ol-summary-row">
                <span>Net (inc. options)</span>
                <span className="v">{fmtGBP(calc.netBeforeVat)}</span>
              </div>
              <div className="ol-summary-row">
              ...
              <div className="ol-summary-row">
                <span>VED + OTR</span>
                <span className="v">{fmtGBP(calc.onRoad)}</span>
              </div>
              ...
              <div className="ol-summary-footer">
                {step === totalSteps - 1
                  ? 'All steps complete — ready to send for e-signature.'
                  : `${totalSteps - step - 1} step${totalSteps - step - 1 === 1 ? '' : 's'} remaining`}
              </div>
            </div>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <div
                  style={{
                    ...
                    height: 26,
                    borderRadius: 6,
                    background: 'var(--ol-cyan-50)',
                    color: 'var(--ol-cyan-600)',
                    display: 'flex',
                    alignItems: 'center',
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--ol-ink-500)', lineHeight: 1.5 }}>
                    {currentStep.id === 'pricing' &&
                      'VAT auto-calculates on net + options (less discount). VED is first-year rate — check band against CO₂.'}
                    {currentStep.id === 'finance' &&
                      'BCH = lease, VAT reclaimable for businesses. Contract Purchase has an optional balloon.'}
                    {currentStep.id === 'customer' &&
                      'Business orders reclaim VAT (where applicable). Personal orders show VAT-inclusive headlines.'}
                    {currentStep.id === 'vehicle' &&
                      'Derivative string should match manufacturer configurator for exact build slot matching.'}
                    {currentStep.id === 'options' &&
                      'Each line carries its own VAT rate. Zero-rated items (e.g. disability adaptations) use 0%.'}
                    {currentStep.id === 'delivery' &&
                      'Driver will confirm delivery window 48 hrs prior. Postcode determines transporter zone fee.'}
                    {currentStep.id === 'review' &&
                      'Customer signs first, then Olaris representative. Envelope returns a fully executed PDF.'}
                  </div>
                </div>
              </div>
            </div>
...
function ReviewBlock({ order, calc }: { order: Order; calc: CalcResult }) {
    ...
    >
      ...
      <span
        style={{
          ...
          fontWeight: 500,
          textAlign: 'right',
          fontFamily: /£/.test(String(v)) ? 'var(--ol-font-mono)' : 'inherit',
        }}
      >
        {v}
      </span>
  ...
  return (
    <div className="ol-grid ol-grid-2" style={{ gap: 20 }}>
      <div>
        <div className="ol-mini" style={{ marginBottom: 8 }}>
          Customer
        </div>
        <Row
          k="Name"
          v={`${order.customer.salutation} ${order.customer.firstName} ${order.customer.lastName}`}
        />
        ...
        <Row
          k="Billing"
          v={`${order.customer.billingAddress}, ${order.customer.billingCity}, ${order.customer.billingPostcode}`}
        />

        <div className="ol-mini" style={{ margin: '20px 0 8px' }}>
          Vehicle
        </div>
      ...
      <div>
        ...
        <Row
          k="Vehicle net"
          v={fmtGBP(order.pricing.vehicleNet - order.pricing.discount)}
        />
        ...
        <div
          style={{
            ...
            justifyContent: 'space-between',
            padding: '12px 0 4px',
            borderTop: '2px solid var(--ol-navy-900)',
            marginTop: 6,
          }}
        >
          <span style={{ fontWeight: 700, color: 'var(--ol-navy-900)', fontSize: 14 }}>
            Drive-away total
          </span>
          <span
            style={{
