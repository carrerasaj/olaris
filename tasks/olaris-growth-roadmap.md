...
- **Schema deploys are a two-step ritual.** For any PR with a new `src/db/migrations/00XX_*.sql`, run `DATABASE_URL=<prod> npm run db:migrate` BEFORE the Vercel deploy goes live — there's no automation linking the two yet. See `tasks/lessons.md` L-001 / L-003 for the gotchas.
- **Credential rotation order: Vercel env first, provider second.** Update `DATABASE_URL` (+ any other server-side env) in Vercel and redeploy BEFORE invalidating the old credential at the provider. Otherwise prod breaks for the rotation window. See `tasks/lessons.md` L-002.

---

## Change log

- **26 Apr 2026** — Infra: drizzle migration journal back-filled on prod (`src/db/scripts/sync-journal-0007-0008.sql` — idempotent template kept in repo for future drift recovery). Build-step migrate (`drizzle-kit migrate && next build`) attempted but reverted (`78519ca`) — Vercel build env didn't surface DATABASE_URL despite the var being configured; durable fix parked behind a future GitHub Action. **Outage:** Neon password rotated provider-side without first updating Vercel env vars; admin login + signing flow + crons all returned 500 (`28P01 invalid_password`) until `DATABASE_URL` / `DATABASE_URL_UNPOOLED` were updated and redeployed. ~30-min window. Lessons captured in `tasks/lessons.md` (L-001..L-004) and saved to memory (`feedback_secrets_rotation.md`).
